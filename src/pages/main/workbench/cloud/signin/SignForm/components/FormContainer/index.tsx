...
import { deleteAttachment, getSubRows, uploadAttachment, cloneAttachment, createData, invokeValidateFunction, getCustomFormJs, invokeLinkageFunction } from "../../service";
...
export default (props) => {
  const { form, sections, setSections, data, customRefForm, customSubmit, layoutRules, params, signValues } = props;
  ...
  const [moduleRule, setModuleRule] = useState([{required: false, message: '请选择' + moduleName}]);
  const [subFormOptions, setSubFormOptions] = useState({});

  useEffect(() => {
    if (signValues.sign_type) {
      // 自动布局
      refForm.setFieldsValue({
        sign_type: signValues.sign_type,
        visit_type: signValues.visit_type,
      });
    }
  }, []);
            ...
            if (item.type === "Fields::NestedFormField") {
              item.fields.forEach(subItem => {
                ...
                if (keyName) {
                  ...
                  fieldMap[key]['exec_formula'].formula = fieldMap[key]['exec_formula'].formula.replace(`/${keyName}/g`, `object['${item.name}']['${subItem.name}']`)
                }
              })
            } else {
              ...
              if (keyName) {
                ...
                fieldMap[key]['exec_formula'].formula = fieldMap[key]['exec_formula'].formula.replace(`/${keyName}/g`, `object['${item.name}']`)
              }
            }
          })
        } 
        ...
        if (res.code === 200 && res.data && res.data.result && typeof res.data.result === 'object') {
          ...
          Object.keys(res.data.result).forEach((key: any) => {
            if (key.indexOf("_SETTING") !== -1) {
              const fieldName = key.replace(/_SETTING/g, "");
              newOverloadData[fieldName] = res.data.result[key];
            } else if (key.indexOf("_ERROR") !== -1) {
              const fieldName = key.replace(/_ERROR/g, "");
  ...
  const executeLayoutRule = (layoutRule, allValues) => {
    const [newSections, clearValues] = getLayoutRuleResult(layoutRule, allValues, sections);
    refForm.setFieldsValue(clearValues);
    // console.log('newSections', newSections, clearValues);
  ...
  const initForm = async () => {
    ...
    if (Object.keys(newAllValues).length > 0) {
      ...
      console.log(mapKeys)
      if (mapKeys.length > 0) {
        const mainDisableEditKeys:any = [];
        const mainHiddenKeys:any = [];
        const subDisableEditKeys:any = {};
        mapKeys.forEach(key => {
  ...
  }, [customModule, actionMapInit]);

  const submitForm = () => {
    refForm.validateFields().then(_values => {
      const allValues = refForm.getFieldsValue(true);
      console.log('allValues', allValues);

      if (allValues?.sign_remark) {
        var reg=/^\d{1,}$/
        var pattern=new RegExp(reg);
        if (pattern.test(allValues.sign_remark)) {
          // message.error('拜访结果不能输入纯数字');
          notification.error({
            message: '拜访结果',
            description: '不能输入纯数字',
          });
      ...
      if (signValues && signValues.sign_type && signValues.sign_type === '签到') {
        if (allValues.module_type) {
          if (!allValues.module_id) {
            message.error(`请选择${allValues.module_type}`);
          } else {
            onFinish(allValues);
          }
        } else {
  ...
  const onFinish = (values: any) => {
    if (form.validate_function_id) {
      invokeValidateFunction({
        ...
        if (res.code === 200 &&  res.data && res.data.result && Object.keys(res.data.result).length > 0) {
          const errors:any = [];
  ...
  const submit = (values) => {
    ...
    if (customSubmit) {
      ...
      let _values;
      if (signValues.sign_type === '签离') {
        _values = {
          ...(signValues || {}),
          ...formatFormData(allValues || {}, fields),
          module_type: signValues.module_type,
          sign_type: signValues.sign_type,
          visit_type: signValues.visit_type,
          sales_sign_in_id: signValues.id,
        }
      } else {
        _values = {
          ...(signValues || {}),
      ...
      Taro.showLoading();
      createData(form.name, {..._values}).then(async (res) => {
        console.log(res);
        if (res.code === 200) {
  ...
  return (
    <FormContainerContext.Provider value={providerValue}>
      <View className="sign-form-container">
        <View className='sign-form-container-scroll'>
          <Form
            form={refForm}
            layout="horizontal"
            labelWrap
            ...
            <Form.Item name="sign_type" hidden />
          </Form>
        </View>
        <View className='sign-form-container-submit'>
          <Button block style={{marginRight: '5px'}} color='warning' onClick={() => refForm.resetFields()}>
            重置
          </Button>
          <Button block loading={loading} style={{marginLeft: '5px'}} color='primary' onClick={submitForm}>
            提交
          </Button>
        </View>
      </View>
