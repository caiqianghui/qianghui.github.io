...
export async function getDataById(model_name, id, params = {}) {
  return HttpLoading.get(`/api/v1/${model_name}/${id}`, {
    params,
  });
}

export async function getFormProfile(model_name) {
  return HttpLoading.get(`/api/v1/${model_name}/profile`, {});
}

export async function getFieldMapDependencies(params) {
  return HttpLoading.get('/api/v1/custom_form/field_map_dependencies', {
    params,
  });
}
export async function getConversionData(model_name, params) {
  return HttpLoading.get(`/api/v1/${model_name}/conversion`, {
    params,
  });
}
