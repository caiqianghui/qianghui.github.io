 ...
 */
 ...
 import { NavBar, Popup, Form, Divider } from "antd-mobile"
 ...
 export default (props: Props) => {
   ...
   const getBluePrint = () => {
     NetCloudFun.get(`/crm/v3/Accounts/${data.id}/actions/blueprint`).then((res: any) => {
      console.log('AccountsBluePrint', res);
       if (res.blueprint) {
         setBlues(res.blueprint.transitions);
         setProcessInfo(res.blueprint.process_info);
       }
     }).catch((err) => {
       console.log(err);
       // Taro.hideLoading();
   ...
   return (
     <View className="content-owner-name">
       {blues ? (
         <>
        <Text className='current-lead-status'>当前状态：{processInfo && processInfo.field_value || '-None-'}</Text>
         <Text>迁移：
 ...
 interface BlueprintProps {
  ...
  const Blueprint = (props: BlueprintProps) => {
   ...
   const onFinish = (values?) => {
     NetCloudFun.put(`/crm/v3/Accounts/${data.id}/actions/blueprint`, {
       blueprint: [values ? {transition_id: blue.id, data: {...values}} : {transition_id: blue.id}],
     }).then((res: any) => {
       console.log('res', res);
       if (res.code === 'SUCCESS' && res.status === 'success') {
           ...
           {fields && fields.length ? fields.map((field, index) => {
             return (
               <>
                ...
                <Text style={{fontSize: '10px', color: '#999'}}>{field.tooltip?.value || null}</Text>
               </>
             )
           }) : (
            <Divider>直接发起</Divider>
           )}
         </Form>
       </View>
     </View>
